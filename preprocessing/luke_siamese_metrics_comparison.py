...
from visualizations import plot_metrics_histogram, plot_model_performance1, plot_metrics_histogram_multiple
...
if __name__ == "__main__":
    ...
    desired_positive_pairs = [1, 3, 5] # selects three positive (and consequentially three negative) pairs per person, can change later
    random_seed = 42 # randomness seed to use. This selects the pairing of images used in the training and test sets
    architecture = 'VGG19'
    metrics_list = []

    for count in desired_positive_pairs:
        print(f"\nRunning experiment with positive_pairs_per_person = {count}")

        # Train the model
        model, history, test_idx, val_idx = train_model(image_paths, identities, positive_pairs_per_person=count, seed=random_seed)
        ...
        optimal_threshold, best_accuracy, metrics = find_optimal_threshold(model, val_paths, val_identities, positive_pairs_per_person=count)
        ...
        evaluation_metrics = evaluate_siamese_network(model, image_paths, identities, threshold=optimal_threshold, positive_pairs_per_person=count)
        ...
        test_metrics = evaluate_test_set(model, test_paths, test_identities, threshold=optimal_threshold, positive_pairs_per_person=count)
